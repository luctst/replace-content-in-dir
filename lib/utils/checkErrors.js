...
module.exports = async (directoryToLoop, directoryToWrite, options) => {
	...
	} else if (directoryToLoop === directoryToWrite) {
		throw new Error(`Cannot overwrite ${directoryToLoop} with ${directoryToWrite}`);
	}
	...
	if (directoryToWrite !== undefined) {
		...
		if (options === undefined) throw new Error("You must enter an options object.");
